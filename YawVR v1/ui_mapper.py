...
class InputDetector(QDialog):
    ...
    def timerEvent(self, event):
        for e in pygame.event.get():
            if e.type not in (pygame.JOYBUTTONDOWN, pygame.JOYAXISMOTION): continue
            ...
            if e.type == pygame.JOYBUTTONDOWN:
                self.detected_input = {
                    ...
                    "phys_input_type": "button", "phys_input_id": e.button
                }
                self.accept()
                ...
                if abs(e.value) > 0.5:
                    self.detected_input = {
                        "phys_device_index": device_index,
                        "phys_device_name": self.joysticks[device_index].get_name(),
                        "phys_input_type": "axis", "phys_input_id": e.axis
                    }
                    self.accept()


class MappingDialog(QDialog):
    """ Revised Editor with Global Start Delay """
    ...
    def __init__(self, mapping_data=None, parent=None):
        ...
        self.txt_desc.setPlaceholderText("e.g. Engine Start Sequence")

        self.spin_start_delay = QSpinBox()
        self.spin_start_delay.setRange(0, 10000)
        self.spin_start_delay.setSuffix(" ms")
        self.spin_start_delay.setToolTip("Wait this long before triggering this action.")

        form_gen.addRow("Description:", self.txt_desc)
        form_gen.addRow("Start Delay:", self.spin_start_delay)
        ...
        # --- AXIS TUNING ---
        grp_tune = QGroupBox("Axis Tuning (Axis Inputs Only)")
        ...
        form_tune.addRow("Max Clamp:", row_cl)
        ...
        self.chk_turbo = QCheckBox("Hold Turbo")
        ...
        form_seq.addRow("Repeats:", self.spin_repeats)
        self.tabs.addTab(self.tab_seq, "Pulse Sequence")

        # TAB 3: Rumble
        self.tab_rumble = QWidget()
        form_rumble = QFormLayout(self.tab_rumble)
        self.lbl_rumble_info = QLabel("Vibrates an axis to simulate engine rumble.")
        self.rumble_target = QComboBox()
        self.rumble_target.addItems(
        ...
        self.rumble_intensity.setRange(1, 100)
        self.rumble_intensity.setValue(10)
        self.lbl_r_int = QLabel("10 %")
        self.rumble_intensity.valueChanged.connect(lambda v: self.lbl_r_int.setText(f"{v} %"))
        row_rint = QHBoxLayout()
        row_rint.addWidget(self.rumble_intensity)
        row_rint.addWidget(self.lbl_r_int)
        self.rumble_speed = QSpinBox()
        self.rumble_speed.setRange(1, 50)
        self.rumble_speed.setValue(10)
        self.rumble_speed.setSuffix(" Hz")
        self.rumble_duration = QSpinBox()
        self.rumble_duration.setRange(0, 10000)
        self.rumble_duration.setValue(0)
        self.rumble_duration.setSuffix(" ms (0 = While Held)")
        form_rumble.addRow(self.lbl_rumble_info)
        form_rumble.addRow("Target Axis:", self.rumble_target)
        form_rumble.addRow("Intensity:", row_rint)
        form_rumble.addRow("Speed (Freq):", self.rumble_speed)
        form_rumble.addRow("Duration:", self.rumble_duration)
        self.tabs.addTab(self.tab_rumble, "Rumble Effect")
    ...
    def update_input_limits(self):
        ...
        if dev_stats:
            count = dev_stats['buttons'] if input_type == "button" else dev_stats['axes']
    ...
    def load_ui_from_data(self):
        ...
        self.spin_start_delay.setValue(int(self.data.get('start_delay', 0)))  # LOAD DELAY
        ...
        atype = self.data.get('action_type')
        ...
        if atype == 'sequence':
            ...
            self.rumble_target.setCurrentText(self.data.get('target', 'left_stick_x'))
            self.rumble_intensity.setValue(int(opts.get('intensity', 0.1) * 100))
            self.rumble_speed.setValue(int(opts.get('speed', 10)))
            self.rumble_duration.setValue(int(opts.get('duration', 0)))
        else:
            self.tabs.setCurrentIndex(0)
            self.combo_action.setCurrentText(atype if atype else 'xbox_button')
    ...
    def save_mapping(self):
        ...
        base_data = {
            ...
            "start_delay": self.spin_start_delay.value(),  # SAVE DELAY
            "tuning": {
        ...
        if tab_idx == 0:
            ...
            if not desc: desc = base_data['target']
        elif tab_idx == 1:
            ...
            if not desc: desc = f"Pulse {base_data['options']['t1']}/{base_data['options']['t2']}"
        elif tab_idx == 2:
            base_data['action_type'] = 'rumble'
            base_data['target'] = self.rumble_target.currentText()
            base_data['options'] = {
                'intensity': self.rumble_intensity.value() / 100.0,
                'speed': self.rumble_speed.value(),
                'duration': self.rumble_duration.value()
            }
            if not desc: desc = f"Rumble {base_data['target']}"
